...
VENV_DIR := $(VENVS_DIR)/albeniz

# Do not remove this block. It is used by the 'help' rule when
# constructing the help output.
# help:
# help: albeniz Makefile help
# help:
...
check-coverage:
	...
	@coverage html -d docs/source/coverage --include="src/albeniz/*"
...
.PHONY: check-types
check-types:
	@cd src; MYPYPATH=$(VENV_DIR)/lib/python*/site-packages mypy -p albeniz --ignore-missing-imports


# help: docs                           - generate project documentation
.PHONY: check-coverage
docs: check-coverage
	@cd docs; rm -rf source/api/albeniz*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./albeniz-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/albeniz-*-py3-none-any.whl
